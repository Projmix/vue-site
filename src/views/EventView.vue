...
export default {
  ...
  methods: {
    async getEvent() {
      ...
      await this.$axios.get(apiUrl, { params }).then(response => {
        ...
        useHead({
          title: (this.event.performance.seo && this.event.performance.seo.seoTitle) ? this.event.performance.seo.seoTitle : this.event.performance.name,
          meta: [
            {
              name: 'description',
              content: (this.event.performance.seo && this.event.performance.seo.seoDescription ? this.event.performance.seo.seoDescription.replace(/<[^>]+>/g, '') : this.event.performance.description.replace(/<[^>]+>/g, ''))
            }
          ]
        });


        if (this.event.calendar && this.event.calendar.length) {
          this.date = this.event.calendar[0].date;
        }
    ...
    getSchedule(date) {
      return new Promise((resolve, reject) => {
        ...
        const apiUrl = `${import.meta.env.VITE_API_URL}/api/v2/schedule/events/${this.$route.params.id}`;
        const params = {
            lang: import.meta.env.VITE_API_LANG,
            jsonld: import.meta.env.VITE_API_JSONLD,
  ...
  beforeMount() {
    ...
    this.layoutLoaded
      ...
      //   });
      // })
      .catch(error => console.error("Ошибка выполнения getEvent", error));
  },
};
</script>
...
<template>
  <main>
      ...
      <div class="hero mv-single-hero noShadow" :style="{ 'background': background }">
        <div class="container">
          <div class="row">
            <div class="col-md-12">
      ...
      <div class="page-single movie-single movie_single" v-if="event.performance">
        <div class="container">
          <div class="row ipad-width2">
            <div class="col-md-4 col-sm-12 col-xs-12">
              <div class="movie-img sticky-sb">
                <img :src="event.performance.image['original']" alt="">
                <div class="movie-btn">	
                  <div class="btn-transform transform-vertical red" v-if="event.performance.video.key">
                    <div><a href="#" class="item item-1 redbtn"> <i class="ion-play"></i> Смотреть трейлер</a></div>
                    <div><a :href="`https://www.youtube.com/embed/${event.performance.video.key}`" class="item item-2 redbtn fancybox-media hvr-grow"><i class="ion-play"></i></a></div>
                  </div>
                  <div class="btn-transform transform-vertical">
                    <div @click.prevent="openWidget(event.performance, 'performance')"><a href="#" class="item item-1 yellowbtn"> <i class="ion-card"></i> Купить билет</a></div>
                    <div @click.prevent="openWidget(event.performance, 'performance')"><a href="#" class="item item-2 yellowbtn"><i class="ion-card"></i></a></div>
                  </div>
                </div>
              </div>
            </div>
            <div class="col-md-8 col-sm-12 col-xs-12">
              <div class="movie-single-ct main-content">
                <h1 class="bd-hd">{{ event.performance.name }}<span></span></h1>
                
                <div class="movie-rate" v-if="event.performance.kinopoiskRank">
                  <div class="rate">
                    <i class="ion-android-star"></i>
                    <p ><span>{{ event.performance.kinopoiskRank.toFixed(1) }}</span> /10<br>
                    ...
                    <ul class="tab-links tabs-mv">
                      ...
                      <div class="tab-content">
                          <div id="overview" class="tab" style="margin-left: -15px;">
                              <div class="row">
                                <div class="col-md-8 col-sm-12 col-xs-12">
                                  <p v-html="event.performance.description"></p>
                                ...
                                <div class="col-md-4 col-xs-12 col-sm-12">
                                  <div class="sb-it" v-if="event.performance.duration">
                                    <h6>Продолжительность: </h6>
                                    <p><a href="#">{{ event.performance.duration }} мин.</a></p>
                                  </div>
                                  <div class="sb-it" v-if="event.performance.minAge">
                                    <h6>Возраст: </h6>
                                    <p><a href="#">{{ event.performance.minAge }}+</a></p>
                                  </div>
                                  <div class="sb-it" v-if="event.performance.showFrom">
                                    <h6>Начало продаж: </h6>
                                    <p><a href="#">{{ strTimestampToHumanTime(event.performance.showFrom) }}</a></p>
                                  </div>
                                  <div class="sb-it" v-if="event.performance.addation.imdbRank">
                                    <h6>Рейтинг IMDb:</h6>
                                    <p><a href="#">{{ event.performance.addation.imdbRank }}</a></p>
                                  </div>
                                  <div class="sb-it" v-if="event.performance.kinopoiskRank">
                                    <h6>Рейтинг кинопоиска:</h6>
                                    <p><a href="#">{{ event.performance.kinopoiskRank }}</a></p>
                                  </div>
                          ...
                          <div id="cast" class="tab active">
                            ...
                            <div class="row">
                              <h3>Выберите дату</h3>
                                <VueDatePicker v-model="date" format="dd.MM.yyyy" id="datePicker" :enable-time-picker="false" hide-offset-dates :allowed-dates="event.calendar.map(item => item.date)" locale="ru" :space-confirm="false" auto-apply></VueDatePicker>
                          ...
                          <div class="mvcast-item" v-if="schedule.data">											
                            <div class="cast-it" v-for="object in schedule.data[0].objects" :key="object">
                              <div class="cast-left" style="width:50%;">
                                <img :src="object.logoUrl['100x50']" alt="">
                                <router-link :to="`/object/${object.id}`">{{ object.name }}</router-link>
                              </div>
                              
                              <div class="cast-right">
                                <button class="widget-btn" v-for="session in object.sessions" :key="session" :disabled="checkSessionDisabled(session)" @click.prevent="openWidget(session, 'session')">
                          ...
                          <div id="media" class="tab">
                            <div class="row">
                              ...
                              <div class="mvsingle-item">
                                <a class="img-lightbox"  data-fancybox-group="gallery" :href="image['880x550']" v-for="image in event.performance.images" :key="image"><img :src="image['300x200']" alt=""></a>
                              </div>
                            </div>
                          </div>
